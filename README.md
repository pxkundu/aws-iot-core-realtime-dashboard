...
This project is configured for easy deployment on AWS Amplify Gen 2. The project includes automation scripts to sync environment variables from your local `.env` file to Amplify Console.

### Quick Deploy

1. **Push your code to GitHub** (main branch)
2. **Connect your repo to AWS Amplify Console**
3. **Set environment variables** using the provided scripts
...
npm run amplify:sync-env

# Using Bash script
npm run amplify:sync-env:bash
```
...
- Local `.env` file with all required variables

### Configuration

Before running the sync scripts, update the configuration in either:
- `scripts/set-amplify-env.sh` (for Bash)
- `scripts/set-amplify-env.js` (for Node.js)
...
The project includes `amplify.yml` for proper build configuration:
...
- Never commit scripts with real App IDs
- Use IAM roles with minimal permissions
- Consider AWS Secrets Manager for sensitive values
